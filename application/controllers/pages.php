   ...
   /* public function view($page = 'home') {
    ...
    public function home($logout = false) {
        ...
        foreach ($dirs as $dir) {
            ...
            foreach ($images as $image) {
                array_push($data [$foldername . '_image_entries'], array('name' => "", 'source' => '/Gioielli-Italia/' . $image, 'thumb' => '/Gioielli-Italia/' . $image . ".thumb"));
            }
            foreach ($subdirs as $subdir) {
                ...
                foreach ($simages as $simage) {
                    array_push($data[$foldername . '_image_entries'], array('name' => "", 'source' => '/Gioielli-Italia/' . $simage, 'thumb' => '/Gioielli-Italia/' . $simage . ".thumb", 'filter' => $subfoldername));
                }
            }
        }
        if ($logout == true) {
    ...
    public function loadAllImages() {
        ...
        foreach ($dirs as $dir) {
            ...
            $images = \glob($dir . "/*.{jpg,png,JPG}", \GLOB_BRACE);
            foreach ($images as $image)
                array_push($data ['image_entries'], array('thumb' => '/Gioielli-Italia/' . $image . ".thumb"));
            foreach ($subdirs as $subdir) {
                $simages = \glob($subdir . "/*.{jpg,png,JPG}", \GLOB_BRACE);
                $subfoldername = str_replace("assets/img/Immagini/" . $foldername . "/", '', $subdir);
                foreach ($simages as $simage)
                    array_push($data ['image_entries'], array('thumb' => '/Gioielli-Italia/' . $simage . ".thumb"));
            }
        }
        return $data;
    }
    ...
    public function deletePhoto() {
        ...
        if (isset($_SESSION['admin']) && $_SESSION['admin'] == true) {
            ...
            $imgpath = \str_replace("/Gioielli-Italia", ".", $imgpath);
